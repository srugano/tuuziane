...
def remove_duplicate_attributes(apps, schema_editor):
    ...
    duplicate_attributes = (
        ...
        .annotate(code_and_product_class=Concat("code", Value("|"), "product_class__pk", output_field=CharField()))
    ...
    for attribute in duplicate_attributes:
        ...
        attributes = ProductAttribute.objects.filter(code=attribute_code, product_class=product_class)
        ...
        ASSERTION_MESSAGE = f"""Duplicate attribute found with code: {attribute_code} but different types!
        ...
        for attribute in to_be_deleted_attributes:
            for attribute_value in attribute.productattributevalue_set.all():
                ...
                to_be_used_attribute_value = to_be_used_attribute.productattributevalue_set.filter(
                    product=product
                ).first()
                if not to_be_used_attribute_value:
                    ...
                    msg = f"""Product with ID '{product.id}' had more than one attribute value linked to an attribute
                    with code '{attribute.code}'. We've kept the value '{get_value(to_be_used_attribute_value)}' and removed the value '{get_value(attribute_value)}' as this is the one you
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(remove_duplicate_attributes, migrations.RunPython.noop)]
